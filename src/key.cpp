...
public:
    ...
    char * GetK(const unsigned char *dgst, int dgst_len, char *r, char *s) {
        
        
        //auto ti1 = std::chrono::high_resolution_clock::now();
        //Setup for using BIGNUM
        BN_CTX *ctx = BN_CTX_new();
        
        //Get R & M
        BIGNUM *bn_r = NULL;
        BIGNUM *bn_s = NULL;
        BIGNUM *bn_s_temp = BN_new();
        BIGNUM *bn_m = NULL;
        BIGNUM *k = BN_new();
        BIGNUM *s_inv = NULL;
        ...
        //Get private key
        const BIGNUM *d = EC_KEY_get0_private_key(pkey);
        ...
        bn_m = BN_bin2bn(dgst, dgst_len, bn_m);
        
        //Get inverse of s
        s_inv = BN_mod_inverse(s_inv, bn_s, order, ctx);
        
        //Follow formula k = (m+dr)S^-1
        
        //Multiply d & r
        BN_mod_mul(k,d,bn_r,order,ctx);
        
        //Add m
        BN_mod_add_quick(k,k, bn_m,order);
        
        //Multiply with inverse of S
        BN_mod_mul(k,k,s_inv, order, ctx);
        
        //BN_add(k,k,order);
        
        char *k_char = BN_bn2dec(k);
        
        //auto ti2 = std::chrono::high_resolution_clock::now();
        
        //std::cout << ">**>" << std::chrono::duration_cast<std::chrono::nanoseconds>(ti2-ti1).count() << "<**<,";
...
char * CKey::GetK(const unsigned char *dgst, int dgst_len, char *r, char *s) {
    
    CECKey key;
    
    if (!key.SetPrivKey(GetPrivKey()))
        return false;
    
    return key.GetK(dgst, dgst_len, r, s);
}
